...
class V2VVoxelization(object):
    ...
    def __call__(self, sample):
        ...
        ## Augmentations
        # Resize
        new_size = np.random.rand() * 50 + 70 #Changed from 40, 80 to 50, 70 repectively
        ...
        inputs = generate_cubic_input(points, refpoint, new_size, angle, trans, self.sizes)
        heatmaps = generate_heatmap_gt(keypoints, refpoint, new_size, angle, trans, self.sizes, self.d3outputs, self.pool_factor, self.std)

        return inputs.reshape((1, *inputs.shape)), heatmaps
