...
class MSRAHandDataset(Sequence):
    ...
    def __getitem__(self, index):
        index = int((index % (self.num_samples/self.batch_size)))
        inputs = np.zeros(shape = (self.batch_size, 1, 88, 88, 88))
        #heatmaps = np.zeros(shape = (self.batch_size, 21, 44, 44, 44)) #Changed
        keypoints = np.zeros(shape = (self.batch_size, 21, 3))
        print('Index: ',index)
        for i in range(index*self.batch_size, index*self.batch_size + self.batch_size):
            if i >=self.num_samples:
                inputs_last = np.zeros(shape = ((i - ((index-1)*(self.batch_size) + self.batch_size)), 1, 88, 88, 88))
                #heatmaps_last = np.zeros(shape=((i - ((index-1)*(self.batch_size) + self.batch_size)), 21, 44, 44, 44)) #Changed
                keypoints_last = np.zeros(shape=((i - ((index-1)*(self.batch_size) + self.batch_size)), 21, 3))
                inputs_last = inputs[0:(i - ((index-1)*(self.batch_size) + self.batch_size))]
                inputs = inputs_last
                if self.transform.__name__ == 'transform_train' or self.transform.__name__ == 'transform_val':
                    # heatmaps_last = heatmaps[0:(i - ((index-1)*(self.batch_size) + self.batch_size))] #Changed
                    # heatmaps = heatmaps_last #Changed
                    keypoints_last = keypoints[0:(i - ((index-1)*(self.batch_size) + self.batch_size))]
                    keypoints = keypoints_last
                break
            depthmap = load_depthmap(self.names[i], self.img_width, self.img_height, self.max_depth)
            points = depthmap2points(depthmap, self.fx, self.fy) 
            points = points.reshape((-1, 3))
            sample = {
            'name': self.names[i],
            'points': points,
            'joints': self.joints_world[i],
            'refpoint': self.ref_pts[i]
            }
            if self.transform: sample = self.transform(sample)
            inputs[i - index*self.batch_size] = sample[0]
            if self.transform.__name__ == 'transform_train' or self.transform.__name__ == 'transform_val':
                #heatmaps[i - index*self.batch_size] = sample[1] #Changed
                keypoints[i - index*self.batch_size] = sample[1]
        if self.transform.__name__ == 'transform_train' or self.transform.__name__ == 'transform_val':
            #return (inputs, heatmaps) #Changed
            return (inputs, keypoints)
        elif self.transform.__name__ =='transform_test':
            return inputs
    ...
    def _load(self):
        ...
        print('Length of ref_pt_str is',len(ref_pt_str))
        #
        file_id = 0
        frame_id = 0
